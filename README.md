...
Uygulama internete de konabilir (GitHub Pages). Sonuç `https` bir adrestir; Android Chrome'da doğrudan **Uygulamayı yükle** çıkar (Chrome bayrağı gerekmez), bilgisayar açık olmadan çalışır. Herkesin kelimeleri **kendi cihazında** kalır; sunucuda kelime, hesap ya da kişisel veri tutulmaz. Kelimeler bilgisayardan telefona kendiliğinden gelir (aşağıdaki eşleştirme); yedek için **Ayarlar → Yedeği indir / yükle** de vardır.
...
6. **Kelimeler kendiliğinden gelir:** her cihazın kelimeleri ayrı saklanır ama **ana kaynak bilgisayardır**. Bilgisayardaki uygulamada eklediğin, düzelttiğin, sildiğin kelime birkaç saniye içinde GitHub'da gizli bir dosyaya yazılır; eşleştirilmiş telefonlar bunu uygulama açıldığında ve açıkken 2 dakikada bir okuyup yansıtır (yeni, düzeltilen, silinen kelimeler; telefondaki "bildim" ilerlemesi ve telefonda kendi eklediğin kelimeler korunur). Her telefonda **yalnızca bir kez eşleştirme** gerekir: bilgisayardaki **Ayarlar → Telefonlarla otomatik eşitleme** bölümündeki bağlantıyı (ya da masaüstündeki `YOKDIL Eslestirme QR.png`) telefonda aç, **Eşleştir**'e bas. Bu bağlantı bir anahtardır: bilen herkes kelime listeni okuyabilir. İnternetteki sürümde de aynı şekilde çalışır (başka Wi-Fi, mobil veri fark etmez). Bilgisayarın internete bağlı ve GitHub girişinin (`gh auth login`) yapılmış olması gerekir.
